...
def createWorklist():
    ...
    for (localKey, localValue) in local:
        source=localValue.split(',')[0] + '/'
        for (remoteKey, remoteValue) in remote:
            target=args.host + ':' + remoteValue.split(',')[0] + '/'
            if localKey == remoteKey:
                ...
                workItem.extend(['rsync', '-av', '--delete', '-PHS'])
                if args.command == 'to':
                    workItem.extend([source, target.translate(str.maketrans({" ": r"\ "}))])
                elif args.command == 'from':
                    workItem.extend([target.translate(str.maketrans({" ": r"\ "})), source])
                else:
