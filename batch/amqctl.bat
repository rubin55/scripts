...
:start
  echo Starting Artemis Messaging Service
  sc start %svc%
  goto :end

:stop
  echo Stopping Artemis Messaging Service
  sc stop %svc%
  goto :end

:status
  echo Showing status for Artemis Messaging Service
  sc query %svc%
  goto :end

:def
  echo Usage: amqctl start^|stop^|status
  echo.
