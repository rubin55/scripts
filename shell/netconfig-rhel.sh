...
STATIC_ROUTES="mgt.${TOPLEVEL}^172.16.0.0/16"
...
rm -f $CONFIG_LOCATION/network-scripts/route-*
for OLD_FILE in `ls $CONFIG_LOCATION/network-scripts/*ifcfg* | grep -v ifcfg-lo`; do
    rm -f $OLD_FILE
done;
        ...
        for DEVICE in `ifconfig -a | grep -i "hwaddr" | awk '{print $1}' | grep -v vmnic`; do
            ...
            if [ "$CHECK_FORMAT" ]; then
                ...
                if [ $? = 0 ]; then
                    ...
                    echo "DEVICE=$DEVICE"              > $CONFIG_LOCATION/network-scripts/ifcfg-$DEVICE
                    echo "BOOTPROTO=static"           >> $CONFIG_LOCATION/network-scripts/ifcfg-$DEVICE
                    echo "ONBOOT=yes"                 >> $CONFIG_LOCATION/network-scripts/ifcfg-$DEVICE
                    echo "HWADDR=$HWADDR"             >> $CONFIG_LOCATION/network-scripts/ifcfg-$DEVICE
                    echo "IPADDR=$DETECTED_ADDR"      >> $CONFIG_LOCATION/network-scripts/ifcfg-$DEVICE
                    echo "NETMASK=$DETECTED_MASK"     >> $CONFIG_LOCATION/network-scripts/ifcfg-$DEVICE
                    echo ""                           >> $CONFIG_LOCATION/network-scripts/ifcfg-$DEVICE
